  ...
  return (
    <div className="row">
      ...
      <div className="col-12 d-flex justify-content-between my-3">
        {mostrar === 0 ? 
          null : // si es 0, no muestra el boton
          (// si es mayor a 0, muestra el boton
            <button className="btn btn-dark" onClick={()=>{setMostrar(mostrar - 8)}}>
              Anterior
            </button>
          )
        }
        <div>{/* Div vacio para que no se desacomode */}</div> 
        {
          productos.length - mostrar <= 8 ? // Hago la resta, si es menro a 8, significa que ya no hay mas productos que mostrar despues
          null :
          (
            <button className="btn btn-dark" onClick={()=>{setMostrar(mostrar + 8)}}>
              Siguiente
